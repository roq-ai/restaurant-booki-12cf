...
import { useRoqClient, useMenuFindFirst } from 'lib/roq';
...
function MenuEditPage() {
  ...
  const { data, error, isLoading, mutate } = useMenuFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: MenuInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.menu.update({
        data: values as RoqTypes.menu,
        where: {
          id,
        },
      });
