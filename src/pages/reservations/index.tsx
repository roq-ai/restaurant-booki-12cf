...
import { useRoqClient, useReservationFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ReservationInterface, unknown>;
...
export function ReservationListPage(props: ReservationListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['special_request.contains'],
          ...params.filters,
        },
        'reservation',
      ),
  ...
  const { data, error, isLoading, mutate } = useReservationFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.reservation.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ReservationInterface) => {
    if (hasAccess('reservation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/reservations/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.reservation_time ? format(record?.reservation_time, 'dd-MM-yyyy') : '',
    },
    { id: 'number_of_people', header: 'number_of_people', accessorKey: 'number_of_people' },
    { id: 'special_request', header: 'Special Request', accessorKey: 'special_request' },
    ...
    hasAccess('table', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'table',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/tables/view/${record.table?.id}`}>
              {record.table?.status}
            </Link>
          ),
        }
