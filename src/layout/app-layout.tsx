...
import { useRestaurantFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'restaurant-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useRestaurantFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Restaurants',
      path: isTenantUser ? `/restaurants/view/${data?.id}` : '/restaurants',
      entity: 'restaurant',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    { name: 'Menus', path: '/menus', entity: 'menu', service: AccessServiceEnum.PROJECT, icon: FiMenu },
    { name: 'Orders', path: '/orders', entity: 'order', service: AccessServiceEnum.PROJECT, icon: FiShoppingCart },
    {
      name: 'Reservations',
      path: '/reservations',
      entity: 'reservation',
    ...
    { name: 'Tables', path: '/tables', entity: 'table', service: AccessServiceEnum.PROJECT, icon: FiLayout },
