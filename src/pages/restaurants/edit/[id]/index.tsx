...
import { useRoqClient, useRestaurantFindFirst } from 'lib/roq';
...
function RestaurantEditPage() {
  ...
  const { data, error, isLoading, mutate } = useRestaurantFindFirst(queryParams, {}, { disabled: !id });
  ...
  const handleSubmit = async (values: RestaurantInterface, { resetForm }: FormikHelpers<any>) => {
    ...
    try {
      ...
      const updated = await roqClient.restaurant.update({
        data: values as RoqTypes.restaurant,
        where: {
          id,
        },
      });
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="restaurant" entityId={id} />
        </FormWrapper>
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <NumberInput
            ...
            formControlProps={{
              id: 'maximum_capacity',
              isInvalid: !!formik.errors?.maximum_capacity,
            }}
            name="maximum_capacity"
            error={formik.errors?.maximum_capacity}
            value={formik.values?.maximum_capacity}
            onChange={(valueString, valueNumber) =>
              formik.setFieldValue('maximum_capacity', Number.isNaN(valueNumber) ? 0 : valueNumber)
            }
          />
