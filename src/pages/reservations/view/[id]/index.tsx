...
import { useReservationFindFirst, useRoqClient } from 'lib/roq';
...
function ReservationViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; restaurant: boolean; table: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useReservationFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('reservation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/reservations/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  ...
                  text={data?.reservation_time ? format(data?.reservation_time, 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Number Of People" text={data?.number_of_people} />

                <FormListItem label="Special Request" text={data?.special_request} />
