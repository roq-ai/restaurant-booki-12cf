...
import { useRoqClient, useTableFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<TableInterface, unknown>;
...
export function TableListPage(props: TableListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useTableFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.table.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: TableInterface) => {
    if (hasAccess('table', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/tables/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'table_number', header: 'table_number', accessorKey: 'table_number' },
    ...
    hasAccess('reservation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.reservation,
        }
      : null,
