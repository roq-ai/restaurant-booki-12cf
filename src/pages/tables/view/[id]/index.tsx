...
import { useTableFindFirst, useRoqClient } from 'lib/roq';
...
function TableViewPage() {
  ...
  const queryParams = useMemo<{ include: { restaurant: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useTableFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('table', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/tables/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Table Number" text={data?.table_number} />

                <FormListItem label="Capacity" text={data?.capacity} />
