...
import { useRoqClient, useOrderFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<OrderInterface, unknown>;
...
export function OrderListPage(props: OrderListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useOrderFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.order.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: OrderInterface) => {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'total_price', header: 'total_price', accessorKey: 'total_price' },
    {
      id: 'order_time',
      header: 'Order Time',
      accessorKey: 'order_time',
      cell: ({ row: { original: record } }: any) =>
        record?.order_time ? format(record?.order_time, 'dd-MM-yyyy') : '',
    },
