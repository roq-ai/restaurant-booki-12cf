...
import { useOrderFindFirst, useRoqClient } from 'lib/roq';
...
function OrderViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; menu: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useOrderFindFirst(queryParams, {}, { disabled: !id });
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem
                  label="Order Time"
                  text={data?.order_time ? format(data?.order_time, 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('menu', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Menu"
                    text={
                      <Link as={NextLink} href={`/menus/view/${data?.menu?.id}`}>
                        {data?.menu?.name + ''}
                      </Link>
                    }
                  />
